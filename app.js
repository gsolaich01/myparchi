...
const _0xSec = (function() {
    ...
    // License key generation algorithm (YOU use this to generate keys)
    // Key = first 12 chars of SHA256(deviceId + bizName + secret)
    async function generateLicenseKey(deviceId, bizName) {
        const combined = deviceId.toUpperCase() + '|' + bizName.toUpperCase().replace(/\s/g, '') + '|' + _s;
    ...
    async function validateLicense(deviceId, bizName, enteredKey) {
        const expectedKey = await generateLicenseKey(deviceId, bizName);
        return enteredKey.toUpperCase() === expectedKey;
    }
    ...
    // Activate license
    function activateLicense(bizName, expiryDays = null) {
        ...
        if (expiryDays) {
            ...
            localStorage.setItem(KEYS.expiry, expiryDate.getTime().toString());
        }
    }

    // Get stored business name
    ...
    overlay.innerHTML = `
        <div class="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-sm">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="activateLicenseKey" placeholder="XXXX-XXXX-XXXX" 
                    class="w-full border-2 border-slate-200 p-3 rounded-xl mt-1 text-center font-bold outline-none focus:border-blue-500 uppercase tracking-widest"
                    maxlength="14"
...
async function attemptActivation(deviceId) {
    ...
    const licenseKey = document.getElementById('activateLicenseKey').value.replace(/-/g, '').trim();
    ...
    // Validate license
    const isValid = await _0xSec.validateLicense(deviceId, bizName, licenseKey);
    
    if (isValid) {
        _0xSec.activateLicense(bizName);
        
        // Show success and reload
        errorEl.innerHTML = '<i class="fa-solid fa-check-circle mr-1 text-emerald-500"></i> <span class="text-emerald-500">Activated! Reloading...</span>';
        errorEl.classList.remove('hidden');
        
        setTimeout(() => location.reload(), 1500);
    } else {
        errorEl.innerHTML = '<i class="fa-solid fa-circle-exclamation mr-1"></i> Invalid license key';
        errorEl.classList.remove('hidden');
